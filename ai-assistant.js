...
class AIWorkflowAssistant {
    ...
    detectIntent(message) {
        const intentPatterns = {
            create_workflow: [
                /create.*workflow/i, /build.*automation/i, /make.*flow/i,
                /set up.*process/i, /automate.*task/i
            ],
            modify_workflow: [
                /change.*workflow/i, /modify.*flow/i, /update.*automation/i,
                /edit.*process/i, /alter.*workflow/i
            ],
            optimize_workflow: [
                /optimize.*workflow/i, /improve.*performance/i, /make.*faster/i,
                /reduce.*time/i, /enhance.*efficiency/i
            ],
            explain_workflow: [
                /explain.*workflow/i, /how.*work/i, /what.*does/i,
                /describe.*process/i, /show.*steps/i
            ],
            troubleshoot: [
                /error/i, /problem/i, /issue/i, /bug/i, /not working/i,
                /failed/i, /broken/i, /fix/i
            ],
            suggest_improvements: [
                /suggest.*improvement/i, /recommend.*change/i, /what.*better/i,
                /how.*improve/i, /make.*better/i
            ]
        };
    ...
    extractEntities(message) {
        ...
        // Service detection
        const services = ['email', 'slack', 'webhook', 'database', 'api', 'http', 'calendar', 'file', 'sms'];
        entities.services = services.filter(service => 
            message.toLowerCase().includes(service)
        );

        // Action detection
        const actions = ['send', 'create', 'update', 'delete', 'notify', 'trigger', 'execute', 'process'];
        entities.actions = actions.filter(action => 
        ...
        entities.data_types = dataTypes.filter(type => 
            message.toLowerCase().includes(type)
        );

        // Frequency detection
        const frequencies = ['daily', 'weekly', 'monthly', 'hourly', 'every'];
        entities.frequencies = frequencies.filter(freq => 
            message.toLowerCase().includes(freq)
        );
    ...
    calculateOverallConfidence(action) {
        ...
        const avgConfidence = action.nodes.reduce((sum, node) => 
            sum + (node.confidence || 0.6), 0) / action.nodes.length;
    ...
    async optimizeWorkflow() {
        ...
        if (actions.length > 3) {
            optimizations.push('Large workflows might benefit from breaking into smaller, focused workflows');
        }

        // Check for missing error handling
        const hasErrorHandling = state.nodes.some(n => n.name.includes('Error') || n.name.includes('Retry'));
