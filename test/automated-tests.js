...
class AutomatedTestRunner {
	...
	async testFileStructure() {
		...
		const requiredDirs = ['.github/workflows', 'docs', 'test'];
	...
	checkJavaScriptSyntax(content) {
		try {
			...
			const requiredFunctions = ['processNaturalLanguage', 'createNode', 'executeWorkflow'];
	...
	generateReport() {
		...
		this.testResults.forEach((result) => {
			...
			console.log(
				`${statusIcon} ${result.test}: ${result.passed} passed, ${result.failed} failed [${result.status}]`,
			);
		});

		console.log('');
		console.log(
			`SUMMARY: ${totalPassed} assertions passed, ${totalFailed} assertions failed across ${totalTests} test suites`,
		);
