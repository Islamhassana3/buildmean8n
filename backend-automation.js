...
class BackendAutomationManager {
    ...
    sendHealthAlert(healthStatus) {
        ...
        if (typeof addMessageToChat === 'function') {
            addMessageToChat('assistant', `🚨 **System Health Alert**
...
${Object.entries(healthStatus.checks)
    ...
    analyzeErrorPatterns(errors) {
        const recent = errors.filter(e => 
            new Date() - new Date(e.timestamp) < 60 * 60 * 1000 // Last hour
        );
        ...
        if (recent.length > 10) {
            ...
            if (suggestions.length > 0 && typeof addMessageToChat === 'function') {
                addMessageToChat('assistant', `⚠️ **Error Pattern Detected**
...
class WorkflowExecutor {
    ...
    async validateNode(node) {
        if (!node.id || !node.type || !node.name) {
            throw new Error(`Invalid node: missing required fields (id: ${node.id}, type: ${node.type}, name: ${node.name})`);
        }
        ...
        try {
            ...
            if (backup.configuration) {
                if (backup.configuration.userPreferences) {
                    localStorage.setItem('userPreferences', JSON.stringify(backup.configuration.userPreferences));
                }
            }
...
class FailureRecoveryAgent {
    ...
    getFailureStats() {
        ...
        const recent = this.failureHistory.filter(f => 
